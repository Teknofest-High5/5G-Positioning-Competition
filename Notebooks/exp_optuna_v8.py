# hyperparameter tuning for cell id combination specific models
import os
...
for cell, neighbors in cell_pairs.items():
    ...
    for neighbor in neighbors:
        ...
        X = df.drop("Latitude", "Longitude", "NR_UE_PCI_0", "NR_UE_Nbr_PCI_0").to_numpy()

        def objective(trial):
            n_estimators = trial.suggest_int("n_estimators", 3, 600)
        ...
        os.makedirs(f"optuna/cell_models/{cell}_{nbr_cell}", exist_ok=True)
        os.makedirs(f"optuna/plots/cell_model_plots/{cell}_{nbr_cell}", exist_ok=True)
        os.makedirs(f"optuna/trial-data/cell_models/{cell}_{nbr_cell}", exist_ok=True)

        with open(f"optuna/cell_models/{cell}_{nbr_cell}/best_params_{timestamp}.pkl", "wb") as f:
            pickle.dump(best_trial.params, f)

        study.trials_dataframe().to_csv(
            f"optuna/trial-data/cell_models/{cell}_{nbr_cell}/optuna_trials_{timestamp}.csv"
        )

        fig_hist = optuna.visualization.plot_optimization_history(study)
        fig_hist.write_html(
            f"optuna/plots/cell_model_plots/{cell}_{nbr_cell}/optimization_history_{timestamp}.html"
        )

        fig_imp = optuna.visualization.plot_param_importances(study)
        fig_imp.write_html(
            f"optuna/plots/cell_model_plots/{cell}_{nbr_cell}/param_importances_{timestamp}.html"
        )
        ...
        fig_slice.write_html(
            f"optuna/plots/cell_model_plots/{cell}_{nbr_cell}/param_slice_{timestamp}.html"
        )
        ...
        if open_webbrowser:
            ...
            webbrowser.open(
                Path(f"optuna/plots/cell_model_plots/{cell}_{nbr_cell}/param_slice_{timestamp}.html")
                .resolve()
                .as_uri()
            )
