# hyperparameter tuning for cell id specific models
import os
...
data = pl.read_parquet("data/parquet/processed/concat_dl_ul_cell_info.parquet").drop_nulls(["Longitude", "Latitude"])
...
for cell in data["NR_UE_PCI_0"].unique():

    print(f"\nModel for pci: {cell}")

    df = data.filter(pl.col("NR_UE_PCI_0") == cell)

    y = df["Latitude", "Longitude"].to_numpy()
    X = df.drop("Latitude", "Longitude", "NR_UE_PCI_0").to_numpy()

    def objective(trial):
        n_estimators = trial.suggest_int("n_estimators", 100, 1000)
        learning_rate = trial.suggest_float("learning_rate", 5e-4, 0.2)
    ...
    study = optuna.create_study(
        direction="minimize",
        sampler=optuna.samplers.TPESampler(n_startup_trials=200, seed=23),
    )
    ...
    os.makedirs(f"optuna/cell_models/{cell}", exist_ok=True)
    os.makedirs(f"optuna/plots/cell_model_plots/{cell}", exist_ok=True)
    os.makedirs(f"optuna/trial-data/cell_models/{cell}", exist_ok=True)

    with open(f"optuna/cell_models/{cell}/best_params_{timestamp}.pkl", "wb") as f:
        pickle.dump(best_trial.params, f)

    study.trials_dataframe().to_csv(
        f"optuna/trial-data/cell_models/{cell}/optuna_trials_{timestamp}.csv"
    )

    fig_hist = optuna.visualization.plot_optimization_history(study)
    fig_hist.write_html(
        f"optuna/plots/cell_model_plots/{cell}/optimization_history_{timestamp}.html"
    )

    fig_imp = optuna.visualization.plot_param_importances(study)
    fig_imp.write_html(
        f"optuna/plots/cell_model_plots/{cell}/param_importances_{timestamp}.html"
    )
    ...
    fig_slice.write_html(
        f"optuna/plots/cell_model_plots/{cell}/param_slice_{timestamp}.html"
    )
    ...
    if open_webbrowser:
        ...
        webbrowser.open(
            Path(f"optuna/plots/cell_model_plots/{cell}/param_slice_{timestamp}.html")
            .resolve()
            .as_uri()
        )
