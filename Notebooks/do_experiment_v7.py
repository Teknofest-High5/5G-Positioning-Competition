# runs base station models with optuna parameters and logs the best scores to a markdown table.
...
with open("model_scores/model_scores_1_cell.md", "w") as f:
    f.write(
...
for cell in data["NR_UE_PCI_0"].unique():
    ...
    X = df.drop("Latitude", "Longitude", "NR_UE_PCI_0")

    param_paths = glob.glob(f"optuna/cell_models/{cell}/best_params_*.pkl")
    os.makedirs(f"optuna/cell_models/{cell}/y_data", exist_ok=True)
    ...
    for param_path in param_paths:
        ...
        if scores.mean() < best_score:
            ...
            with open(f"optuna/cell_models/{cell}/final_params.pkl", "wb") as f:
            ...
            for fold_idx in range(n):
                ...
                pl.DataFrame(
                    ...
                    f"optuna/cell_models/{cell}/y_data/fold_{fold_idx}.parquet"
                )
            ...
            pl.DataFrame(feature_importances).write_parquet(
                f"optuna/cell_models/{cell}/feature_importances.parquet"
            )
    ...
    with open("model_scores/model_scores_1_cell.md", "a") as f:
        f.write(
            f"| {cell} | {best_scores.mean():.4f} | {max(best_scores):.4f} | {best_max_scores.mean():.4f} | {max(best_max_scores):.4f} |\n"
        )
