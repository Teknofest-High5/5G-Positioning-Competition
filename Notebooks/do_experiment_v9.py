...
with open("model_scores/model_scores_3_cells.md", "w") as f:
    f.write(
...
for combination in cell_combinations:
    ...
    X = df.drop(
        "Latitude", "Longitude", "NR_UE_PCI_0", "NR_UE_Nbr_PCI_0", "NR_UE_Nbr_PCI_1"
    )

    best_score = float("inf")

    param_paths = glob.glob(
        f"optuna/cell_models/{cell}_{nbr_cell_0}_{nbr_cell_1}/best_params_*.pkl"
    )
    os.makedirs(
        f"optuna/cell_models/{cell}_{nbr_cell_0}_{nbr_cell_1}/y_data", exist_ok=True
    )
        ...
        if scores.mean() < best_score:
            ...
            for fold_idx in range(n):
                ...
                pl.DataFrame(
                    ...
                    f"optuna/cell_models/{cell}_{nbr_cell_0}_{nbr_cell_1}/y_data/fold_{fold_idx}.parquet"
                )
            ...
            pl.DataFrame(feature_importances).write_parquet(
                f"optuna/cell_models/{cell}_{nbr_cell_0}_{nbr_cell_1}/feature_importances.parquet"
            )

    with open(
        f"optuna/cell_models/{cell}_{nbr_cell_0}_{nbr_cell_1}/final_params.pkl", "wb"
    ) as f:
    ...
    with open("model_scores/model_scores_3_cells.md", "a") as f:
        f.write(
            f"| {cell} | {nbr_cell_0} | {nbr_cell_1} | {best_scores.mean():.4f} | {max(best_scores):.4f} | {best_max_scores.mean():.4f} | {max(best_max_scores):.4f} |\n"
        )
